...
| 赛题要求 | 本项目实现 |
| --- | --- |
| 使用 MinerU 工具链 | `mineru_client.py` 调用 MinerU SaaS 精准解析 API，`agent.py` 兼容 `full_zip_url` 与 `markdown_url` |
...
其中前三项不依赖真实外部密钥，`run_real_pdf_test.py` 需要 MinerU 与 DeepSeek 密钥。
...
| 文件 | 说明 |
| --- | --- |
| `main.py` | FastAPI 服务入口、异步任务和状态查询 |
| `agent.py` | 多场景 Agent、上下文打包、财务规则抽取、LLM 调用和审计校验 |
| `mineru_client.py` | MinerU SaaS 任务提交与轮询 |
| `env_setup.py` | `.env` 自动加载 |
| `README.md` | 项目说明、启动方式和 API 示例 |
| `TECHNICAL_REPORT.md` | 技术报告 |
| `TESTING.md` | 测试矩阵与稳定性验证 |
| `EXECUTION_LOGS.md` | 运行日志与测试摘要 |
| `SUBMISSION_PACKAGE.md` | 提交材料清单 |
| `run_real_pdf_test.py` | 真实 PDF 联调脚本 |
| `test_local_finance.py` | 本地烟测 |
| `test_agent_unit.py` | Agent Markdown 兼容与财务校验单元测试 |
| `test_stability.py` | 并发稳定性测试 |

## 7. 提交状态

项目已经具备可提交的代码、文档、测试和 API 接入能力。正式提交前只需在部署环境写入真实 `.env` 密钥，并重新运行真实 PDF 联调以生成最新日志。
